...
def home():
    st.title("🏠openai playground by chenyunmolu")
    st.caption(
    ...
    # 添加手动更新base_url和api_key的方式
    st.session_state.base_url = st.sidebar.text_input("base_url", value=st.session_state.base_url)
    st.session_state.api_key = st.sidebar.text_input("api_key", value=st.session_state.api_key, type="password")

    # 添加语言单选框
    option = st.radio("Change Language : ", ("En", "Zh"), index=1, horizontal=True)
    if option == "Zh":
        st.markdown(
            """
            **体验OpenAI多模态功能**
